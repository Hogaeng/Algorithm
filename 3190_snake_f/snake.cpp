...
using namespace std;
void printstat(vector<Part> snake,int bn,int apples[100][2],int an){
	bool snflag=false;
	bool anflag=false;
	for(int i=0;i<bn;i++){
		for(int j=0;j<bn;j++){
			for(int c = 0;c<snake.size();c++){
				if(i==snake[c].curs[0]&&j==snake[c].curs[1]){
					snflag=true;
					if(c==0)
						cout<<snake.size();
					else if(c==snake.size()-1)
						cout<<"*";
					else
						cout<<"O";
					break;
			...
			if(!snflag){
				for(int c=0;c<an;c++){
					if(i==apples[c][0]-1&&j==apples[c][1]-1){
						cout<<"@";
						anflag=true;
					}
				}
...
void roll(int curs[2],int &ndir, int tdir){
	if(tdir == D){
		ndir++;
		if(ndir>3)
			ndir=0;
...
void eatA(vector<Part> &snake,Part &tpart){
...
bool rollsnake(vector<Part> &snake,bool aflag){
	...
	if(chekboarder(snake,100))//자기꼬리를 무는가
		return true;
	for(int i=1;i<sn;i++){
		ttdir1 = snake[i].tdir;
		tndir1 = snake[i].ndir;
...
bool rollbodysnake(vector<Part> &snake,int sn)
{
	int ttdir1, tndir1;
	int ttdir2, tndir2;
	ttdir1=snake[0].tdir;
...
int main(){
	freopen("snake_input_t.txt","r",stdin);
	int tmp;
	cin>>tmp;
	for(int z=0;z<tmp;z++){
		int bn;
